...
export default function Home() {
    const directionsRendererRef = useRef(null); // <- Add this line
    ...
    const [radius, setRadius] = useState(5000); // Reduced radius
        ...
        (map, keyword, setState) => {
            ...
            service.nearbySearch(request, (results, status) => {
                if (status === google.maps.places.PlacesServiceStatus.OK) {
                    ...
                    console.log(`Error fetching ${keyword}:`, status);
                    setState([]);
                }
            });
        },
    ...
    const updateData = useCallback(
        ...
        [fetchPlaces, userLocation] // Re-fetch when userLocation changes
    );
    ...
    setUserLocation={(location) => {
        setUserLocation(location);
        setInitialFetchComplete(false); // Reset to trigger fetch on location change
    }}
    setIsLoading={setIsLoading}
    directionsRendererRef={directionsRendererRef} // <- Pass this correctly
